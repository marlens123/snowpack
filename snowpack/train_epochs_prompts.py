...
def multiclass_epoch(train_loader, predictor, accumulation_steps, epoch, 
    ...
    for batch_idx, tup in enumerate(train_loader):
        ...
        prd_mask = prd_masks.mean(dim=0, keepdim=True)
        ...
        loss = F.cross_entropy(prd_mask, gt_mask, weight=class_weights)
        
        # IoU computation
        # pred_labels = torch.argmax(prd_masks, dim=1)  # Shape: [batch_size, H, W]
        pred_labels, _ = prd_masks.max(dim=0, keepdim=True) 
...
class MulticlassSAMWrapper(nn.Module):
    ...
    def forward(self, sparse_embeddings, dense_embeddings, high_res_features, feats, batched_mode):
